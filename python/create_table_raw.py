...
def main(matrix_file,infile,cutoff):
    ...
    for i in range(seq_length+1):
        if dist_counts[i]==0:
            #print("0.000")
            outstr.append("0.000000")
        else:
            #freq=float(dist_counts[i])/float(counter)
            outstr.append("{:d}".format(dist_counts[i]))
            #outstr.append("1_in_{}".format(round(float(counter)/float(dist_counts[i]))))
    print("\t".join(outstr))
    return
