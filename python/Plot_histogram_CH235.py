...
def plot_cum_heatmap(outpdf,data):
    ...
    ax.set_xticks(range(0,14))
    ax.set_xticklabels(["$\leq${}".format(x) for x in range(0,14)])
    ax.grid(b=None)
    plt.grid(b=None)
...
def plot_legend(outpdf):
    ...
    for i,data in enumerate([10,100,1000,10000,100000,1000000,10000000,100000000,1000000000,0]):

        if data==0:
            color=[1,1,1]
            ax.text(1.5,i+0.5,"no data".format(data))
        else:
            color=color_lookup(1.1/data)
...
def main(filename,outname):
    ...
    for T in threshold_list:
        distanceHash[T]=numpy.zeros([1,14])
    ...
    heatmap=numpy.zeros([len(threshold_list),14])
    ...
    for i,T in enumerate(threshold_list):
        #heatmap[i,:]=GenerateBins(distanceHash[T])
        #fullCount=numpy.sum(distanceHash[T])
        fullCount=9623490#functional, uniq briney number
