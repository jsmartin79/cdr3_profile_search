...
{
  for(int i=reading_frame-1; i<=dna.size()-3; i+=3)
    {
...
void sequence_identity_semi_global(string alignment_seq1, string alignment_seq2, double &identity)
{
  ...
  //define start and end stretch of gaps
  int s1_align_start=0, s2_align_start=0;
  for(int j=0; j<alignment_seq1.size(); j++)
    {
      if (alignment_seq1[j] != '-'){break;}
      s1_align_start++;
    }
  for(int j=0; j<alignment_seq1.size(); j++)
    {
      if (alignment_seq2[j] != '-'){break;}
      s2_align_start++;
    }
  int s1_align_end=0, s2_align_end=0;
  for(int j=alignment_seq1.size()-1; j>0; j--)
    {
      if (alignment_seq1[j] != '-'){break;}
      s1_align_end++;
    }
  ...
  for(int j=0; j<alignment_seq1.size(); j++)
    {
      if ((j>=s1_align_start) && (j>=s2_align_start) && (j<=alignment_seq1.length()-s1_align_end-1) && (j<=alignment_seq2.length()-s2_align_end-1)) //ignore gapped ends
	{
	  if (alignment_seq1[j]==alignment_seq2[j]){match_count++;}
	  total_count++; 
	}
    }
  identity=(match_count/(double) total_count)*100.0;
